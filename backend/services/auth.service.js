...
class AuthService {
  static async login(userId, userPw) {
    try {
      ...
      if (!findUser) throw new CustomError('not exist user, plz check your id', 401)
      if (!(await bcryptHelper.isCmp(userPw, findUser.u_pw)))
        throw new CustomError("doesn't correct your password,plz check your password ")
  ...
  static async join(user) {
    try {
      ...
      if (!(await userService.createUser(user))) throw new CustomError('when create user, expect error', 401)
      return true
    } catch (err) {
      throw err
    }
  ...
  static validExistTokenAndGetToken(userId) {
    return new Promise((resolve, reject) => {
      connection.query(`select * from token where u_id = '${userId}'`, (err, result) => {
        if (err) reject(err)
        resolve(result[0])
      })
    })
  ...
  static saveToken(userId, token) {
    return new Promise((resolve, reject) => {
      connection.query('insert into token values(?,?)', [userId, token], (err, result) => {
        if (err) reject(err)
  ...
  static removeToken(token) {
    return new Promise((resolve, reject) => {
      connection.query(`delete from token where accessToken = '${token}'`, (err, result) => {
        if (err) reject(err)
      })
    })
  }
