    ...
    return new Promise((resolve, reject) => {
      connection.query(
        ...
        RIGHT JOIN (
            ...
            FROM room
            GROUP BY r_id
        ) AS room_data ON user_data.r_id = room_data.r_id)as cnt_table where room.r_id = cnt_table.r_id and r_headcount != cnt_table.total_headcount`,
        (err, result) => {
          if (err) reject(err);
          resolve(result);
        }
