...
class UserService {
  ...
  async findUser(userId = null, userPw = null) {
    try {
      ...
      if (!userPw) return this.users.find((user) => user.u_id == userId) || null
      return this.users.find((user) => user.u_id == userId && user.u_pw == userPw) || null
    } catch (err) {
      throw err
    }
  }
  ...
  createUser(user) {
    return new Promise(async (resolve, reject) => {
      ...
      connection.query(this.sql, [user.userId, hashedPw, user.userName, user.birthDate], (err, result) => {
        if (err) reject(err)
